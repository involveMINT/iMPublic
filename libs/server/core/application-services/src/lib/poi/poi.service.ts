...
export class PoiService {
  ...
  async addTags(query: IQuery<Poi>, token: string, dto: AddTagsDto) {
    const user = await this.auth.validateUserToken(token ?? '', { changeMaker: { id: true } });
    const poi = await this.poiRepo.findOneOrFail(dto.poiId, {
      enrollments: { changeMaker: { id: true }, project: { id: true } },
    });
    ...
    return this.poiRepo.update(dto.poiId, { id: dto.poiId, enrollments: [...dto.enrollmentIds] }, query);
  }

  async getTags(query: IQuery<Poi>, token: string, dto: GetTagsDto) {
    const user = await this.auth.validateUserToken(token ?? '', { changeMaker: { id: true } });
    const poi = await this.poiRepo.findOneOrFail(dto.poiId, {
      enrollments: { changeMaker: { id: true, firstName: true, lastName: true } },
      id: true,
    });
    ...
    return poi.enrollments.map((e) => e.changeMaker.id);
  }
}
