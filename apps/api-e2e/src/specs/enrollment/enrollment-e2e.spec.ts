...
describe('ExchangePartner Orchestration Integration Tests', () => {
  ...
  describe('get', () => {
    ...
    it('should get enrollment after applying', async () => {
      ...
      await axios.post('http://localhost:3335/enrollment/startApplication', {
        query: {},
        token: auth.body.token,
        dto: {
  ...
  describe('linkPassportDocument', () => {
    it('should link passport document', async () => {
      ...
      // );
      const { body: res } = await axios.post('http://localhost:3335/enrollment/linkPassportDocument', {
        query: {
          enrollmentDocuments: {
            passportDocument: {
              id: true,
        ...
        dto: {
          ...
          projectDobumentId: projectDocId,
        },
      });
  ...
  describe('processEnrollmentApplication', () => {
    it(`should not process application if not in pending state`, async () => {
      ...
      //   enrollmentId: body.id,
      // });
      const { error } = await axios.post('http://localhost:3335/enrollment/processEnrollmentApplcation', {
        query: {},
        token: auth.body.token,
        dto: {
          approve,
    ...
    it(`should approve application`, async () => {
      ...
      // );
      const { body: res } = await axios.post(
        'http://localhost:3335/enrollment/processEnrollmentApplication',
        {
          query: {
            dateApplied: true,
            dateApproved: true,
  ...
  describe('revert', () => {
    it('should revert enrollment', async () => {
      ...
      //   { enrollmentId: body.id }
      // );
      const { body: revert } = await axios.post(
        'http://localhost:3335/enrollment/revertEnrollmentApplication',
        {
          query: {
            dateApplied: true,
            dateApproved: true,
  ...
  describe('retire', () => {
    it('should not retire if not yet approved', async () => {
      ...
      //   enrollmentId: body.id,
      // });
      const { error } = await axios.post('http://localhost:3335/enrollment/retireEnrollment', {
        query: {},
        token: auth.body.token,
        dto: {
          enrollmentId: body.id,
